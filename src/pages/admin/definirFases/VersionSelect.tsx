...
import { Version } from "../definirFases/page";
...
export function Versiones({ versiones }: { versiones: Version[] }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6">
      {versiones.map((event) => (
        <Link to={`/admin/version/${event.id}/fechas`} key={event.id}>
          <Card key={event.id} className="flex flex-col h-full hover:text-primary hover:border-primary border-2" >
