        ...
        <DialogContent className="sm:max-w-[700px] max-h-10/12 overflow-y-auto">
          ...
          <div className="max-h-[300px] overflow-y-scroll">
            <Table>
              ...
              <TableBody className="">
                {listas.map(
                  ({ codigo_lista, postulantes_count, created_at }, i) => (
                    <TableRow
                      key={i}
                    ...
                    >
                      <TableCell>{codigo_lista}</TableCell>
...
const ModalConfirmarMasPostulantes = ({
  ...
  return (
    <AlertDialog open={openConfirm}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>¿Desea Registrar más postulantes?</AlertDialogTitle>
          <AlertDialogDescription>
            Presione continuar para registrar mas postulantes en una inscripcion
            Presione finalizar para terminar el registro
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <ButtonFinalizarRegistro show onFinish={onFinish} />
          <Link to={codigo_lista + ""}>
            <Button variant={"secondary"} className="w-full">
              {" "}
              <Plus />
              Continuar Inscribiendo más postulantes
            </Button>
          </Link>
        </AlertDialogFooter>
      </AlertDialogContent>
  ...
  return (
    <Dialog open={openCode} onOpenChange={setOpenCode}>
      <DialogContent className="flex flex-col items-center p-4 ">
        ...
        <p className="font-bold text-6xl text-primary">{codigo_lista}</p>
        ...
        <Button asChild variant="outline" className="w-full mb-2">
          <Link
            to={`https://wa.me/?text=${encodeURIComponent(
              "Su codigo de inscripcion es el siguiente: *" + codigo_lista + "*"
            )}`}
            target="_blank"
          >
