...
const DescargarPlantilla = ({ olimpiada }: { olimpiada: Olimpiada }) => {
  ...
  } = useAreasConCategorias(olimpiada.id, {
    queryKey: ["areasConCategorias", olimpiada.id],
    enabled: fetchData,
  });

  const isDataLoading =
    isLoadingColegios || isLoadingDepartamentos || isLoadingAreasCategorias;
  const isDataError =
    isErrorColegios || isErrorDepartamentos || isErrorAreasCategorias;
