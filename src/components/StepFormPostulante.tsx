...
const PersonalStep = ({
  ...
  onNext: (data: PersonalData & ExtraData, postulante?: PostulanteData) => void;
}) => {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <ComboBox
                  disabled={ubicacionesLoading || initialData?.fieldsDisabled}
                  ...
                  onChange={(val) => {
                    field.onChange(val);
                    setSelectedDepartamento(Array.isArray(val) ? val[0] : val);
                  }}
                />
              </FormControl>
              <FormMessage />
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <ComboBox
                  disabled={
                    !selectedDepartamento || initialData?.fieldsDisabled
                  }
                  values={provincias
                    .filter(
                      (p) =>
                        p.departamento_id.toString() === selectedDepartamento
                    )
                    .map((p) => ({
                      id: p.id.toString(),
                      nombre: p.nombre,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submit)} className="space-y-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>¿A quién pertenece el teléfono?</FormLabel>
              <FormControl>
                <ComboBox
                  values={CONTACTOS}
                  value={field.value}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} type="email" placeholder="Ingrese correo" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
const ContactoStep = ({
  ...
  const [contactos, setContactos] = useState<ContactoData[]>(initialData || []);
  ...
  const handleAddContacto = (data: ContactoData) => {
    if (
      ...
      contactos.some(
        ({ telefono_contacto }) => telefono_contacto == data.telefono_contacto
      )
    ) {
    ...
    <>
      ...
      <div className="mt-6 space-y-4">
        {contactos.map((contacto, index) => {
          const tipoTel =
            CONTACTOS.find((c) => c.id === contacto.tipo_contacto_telefono)
              ?.nombre || "—";
          const tipoEmail =
            CONTACTOS.find((c) => c.id === contacto.tipo_contacto_email)
              ?.nombre || "—";
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-medium">
                      <strong>Tipo de contacto telefónico :</strong> {tipoTel}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span>
                      <strong>Correo:</strong> {contacto.email_contacto}
                    </span>
                  </div>

                  <div className="flex items-center gap-2">
                    <strong>Tipo de contacto correo :</strong>{" "}
                    <span className="font-medium">{tipoEmail}</span>
                  </div>
                </div>
                {index != 0 && (
                  <Button
            ...
            let categoriasAreas =
              categorias?.flatMap(
                ({ id: idCat, nombre: nombreCat, areas }) =>
                  areas?.map(({ id: idArea, nombre: nombreArea }) => ({
                    id: `${idArea}-${idCat}`,
                    nombre: `${nombreArea} - ${nombreCat}`,
                  })) || []
              ) || [];
            if (initialData?.niveles_inscritos) {
              categoriasAreas = categoriasAreas.filter(({ id }) => {
                const encontrado = initialData.niveles_inscritos?.find(
                  ({ id_area, id_cat }) => `${id_area}-${id_cat}` == id
                );
                return !encontrado;
              });
            }
              ...
              <FormItem>
                <div className="flex justify-between">
                  ...
                  {initialData?.niveles_inscritos && (
                    <ModalIncritos initialData={initialData} />
                  )}
                </div>
                <FormControl>
                  <MultiSelect
                    ...
                    max={
                      ...
                      0 - (initialData?.niveles_inscritos?.length ?? 0)
                    }
                    values={categoriasAreas}
                    value={
                      field.value?.map(
                        ({ id_area, id_cat }) => `${id_area}-${id_cat}`
                      ) || []
                    }
                    onChange={(selected: string[]) => {
                      const transformed = selected.map((item) => {
                        const [idArea, idCat] = item.split("-").map(Number);
                        console.log(item);
                        return {
                          id_cat: idCat,
                          id_area: idArea,
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-md">
            El Postulante esta registrado a las siguiente Areas Categorias:
          </DialogTitle>
          <DialogDescription>
            {inscritos.length > 0 ? (
              <ul className="mt-2 space-y-2">
                {inscritos.map(({ nivel_competencia }, idx) => (
                  <li key={idx} className="flex justify-between">
                    <span className="font-medium">
                      {nivel_competencia.split("-").slice(0, -1)}
                    </span>
...
const StepFormPostulante = ({
  ...
  const [openConfirmModification, setOpenConfirmModification] = useState(false);
    ...
    <div className="w-full h-full flex flex-col justify-around mx-auto">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              ¿Estas seguro de que desea modificar los siguiente datos?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Se reemplazarán los datos de las otras inscripciones por los
              siguientes:
            </AlertDialogDescription>
                ...
                <li key={index}>
                  ...
                  {cambio.campo === "Curso" && (
                    <Alert className="" variant={"destructive"}>
                      ...
                      <AlertDescription>
                        se eliminarán las inscripciones que no pertenecen a este
                        grado.
                      </AlertDescription>
                    </Alert>
                  )}
...
const obtenerCambios = (
  ...
  if (postulante.id_departamento != data.departamento) {
    cambios.push({
      ...
      anterior:
        departamentos.find(
          ({ id }) => id.toString() === postulante.id_departamento.toString()
        )?.nombre || "",
      nuevo:
        departamentos.find(
  ...
  if (postulante.id_provincia != data.provincia) {
    cambios.push({
      ...
      anterior:
        provincias.find(
          ({ id }) => id.toString() === postulante.id_provincia.toString()
        )?.nombre || "",
      nuevo:
        provincias.find(({ id }) => id.toString() === data.provincia.toString())
          ?.nombre || "",
    });
  }
  if (postulante.id_colegio != data.colegio) {
    cambios.push({
      ...
      anterior:
        colegios.find(
          ({ id }) => id.toString() === postulante.id_colegio.toString()
        )?.nombre || "",
      nuevo:
        colegios.find(({ id }) => id.toString() === data.colegio.toString())
          ?.nombre || "",
    });
  }
