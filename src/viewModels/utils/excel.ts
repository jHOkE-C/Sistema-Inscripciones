...
import { CONTACTOS, GRADOS } from "@/models/interfaces/postulantes";
...
export const generarExcel = async (
  ...
  const SheetPlantilla = async () => {
    ...
    for (let index = 1; index <= limiteInscripciones; index++) {
      columns = [
        ...columns,
        {
          name: "Área categoría " + index + (index > 1 ? " (opcional)" : " "),
        },
      ];
    }
    ...
    for (let i = 2; i <= numFilas + 1; i++) {
      ...
      for (let index = 0; index < limiteInscripciones; index++) {
        const col = 14 + index;
        ws.getCell(`${colLetter(col)}${i}`).dataValidation = {
          ...
          prompt:
            "Seleccione un Area-Categoria si no aparece una lista entonces no hay areas a las que se pueda inscribir o cambie de grado",
        };
      }
    }
  ...
  saveAs(new Blob([buf]), `${nombre ? nombre : "plantilla_inscripcion"}.xlsx`);
};

const SheetList = async (
  wb: Workbook,
  colegios: Colegio[],
  areasCategorias: Categoria[]
) => {
