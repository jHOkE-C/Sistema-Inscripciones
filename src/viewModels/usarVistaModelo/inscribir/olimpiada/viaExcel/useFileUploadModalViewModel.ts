...
import { CategoriaExtendida, UploadResponse, newExcelPostulante, newPostulante } from "@/models/interfaces/postulante.interface";
import { ValidationError, ErroresDeFormato } from "@/models/interfaces/error.interface";
...
export const useFileUploadModalViewModel = ({ olimpiada, onSubmit = () => {} }: UseFileUploadModalProps) => {
    ...
    const inscribirPostulantes = async (postulantes: newPostulante[]) => {
        ...
        try {
            ...
            await axios.post<UploadResponse>(`${API_URL}/api/inscripciones/bulk`, payload);
